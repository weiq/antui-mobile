...
export class Native {
  constructor(opts = {
    ...
    if (opts.func) {
      ...
      } else if (typeof opts.func === "object" && (!!opts.func.ios || !!opts.func.android)) {
    ...
    window[opts.nameCB] = (res) => {
      trigger(this.NATIVE_IDENT, res);
    };
  }

  activate = (param) => {
    const device = Session.getAttribute("device");
    return new Promise((resolve, reject) => {
      once(this.NATIVE_IDENT, (res) => this.handleData(res, device, resolve, reject));
